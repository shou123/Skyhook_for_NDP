...
int main() {
  ...
  outfile.open("/home/yue21/skyhookdm/scripts/deploy/example_result.txt"); // opening file "result.txt"
  
  std::cout << "the file create." << std::endl;

  outfile << table->ToString();

  outfile.close();
  std::cout << "example_result saved to file." << std::endl;
}
